...
def get_creds_from_sqlite(db_path, table_name):
    ...
    try:
        ...
        with sqlite_engine.connect() as conn:
            creds_df = pd.read_sql(f"SELECT url as host, port, \"user\", \"pass\" as password FROM {table_name}", conn) 
        ...
        if not all(key in creds for key in required_keys):
            print(f"  [Load Ошибка] В creds.db не хватает данных. Нужны: {required_keys}")
            return None
...
def load_all_data(transformed_data: dict[str, pd.DataFrame], root_dir: str): 
    ...
    for file_name, df in transformed_data.items():
        ...
        # Валидация
        if df.empty:
            print(f"  [Load Предупреждение] DataFrame {file_name} пуст. Валидация не пройдена. Пропускаем.")
        ...
        if 'count' in df.columns and db_ready:
            ...
            if "roots" in table_name:
                ...
                print(f"  [Load] Пропускаем загрузку {file_name} в БД (загружаем только 'roots').")

        elif 'count' not in df.columns and db_ready:
             print(f"  [Load] Пропускаем загрузку {file_name} в БД (это не 'counts' файл).")
