...
def download_and_convert_to_csv(file_id: str, output_path: str):
    ...
    temp_xls_path = os.path.join(temp_dir, os.path.basename(output_path).replace(".csv", ".xls"))
    ...
    if not os.path.exists(output_path):
        ...
        try:
            ...
            # Валидация
            if df.empty:
                print(f"  [Extract Ошибка] Файл {temp_xls_path} пуст. Валидация источника не пройдена.")
                return False
